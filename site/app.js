...
if(couponForm){const qp=new URLSearchParams(location.search);['utm_source','utm_medium','utm_campaign'].forEach(k=>couponForm.elements[k].value=qp.get(k)||'direct');couponForm.addEventListener('submit',async e=>{e.preventDefault();const status=couponForm.querySelector('.form-status'),button=couponForm.querySelector('button[type=submit]');button.disabled=true;status.textContent='Registering your reader access…';const data=new FormData(couponForm);try{const response=await fetch(couponForm.action,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(data).toString()});if(!response.ok)throw new Error(`form ${response.status}`);const email=String(data.get('email')).trim().toLowerCase(),bytes=new TextEncoder().encode(`signal60:${email}`),hash=await crypto.subtle.digest('SHA-256',bytes),token=[...new Uint8Array(hash)].slice(0,4).map(x=>x.toString(16).padStart(2,'0')).join('').toUpperCase();document.querySelector('#coupon-code').textContent=`SIGNAL60-${token}`;couponForm.hidden=true;document.querySelector('#coupon-result').hidden=false;}catch{status.textContent='The signal could not be registered. Please try again.';button.disabled=false}});document.querySelector('#copy-coupon').onclick=()=>{navigator.clipboard.writeText(document.querySelector('#coupon-code').textContent);toast('Coupon copied.')}}
